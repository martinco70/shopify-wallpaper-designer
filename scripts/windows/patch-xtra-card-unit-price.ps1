...
) | ForEach-Object { Join-Path $ThemeDir $_ }
...
foreach($file in $existing){
  ...
  if($hasRender){
    ...
    if($renderIdx -ge 0){
      ...
      $imageHits = $imageMarkers | Where-Object { $before -match $_ }
      $infoHits = $infoMarkers | Where-Object { $before -match $_ }
      if($imageHits -and (-not $infoHits)){
        $needsRelocate = $true
      }
    }
  }
  ...
  $insertion = "\n{% render 'wpd-price-per-unit', product: card_product, variant: card_product.selected_or_first_available_variant %}\n"
